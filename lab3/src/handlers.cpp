...
HandlerBase::HandlerBase(const components::ComponentConfig& config, const components::ComponentContext& context)
    : HttpHandlerBase(config, context), storage_(context.FindComponent<RecipeStorage>()) {}
...
std::string RootHandler::HandleRequestThrow(
  ...
  builder["service"] = "recipe-manager-userver";
  builder["status"] = "ok";
  builder["variant"] = "23";
  builder["description"] = "Recipe management API (in-memory, 3-database architecture)";
...
CreateUserHandler::CreateUserHandler(const components::ComponentConfig& config, const components::ComponentContext& context)
    : HttpHandlerJsonBase(config, context), storage_(context.FindComponent<RecipeStorage>()) {}
